...
## 10. Perfis de Artista e materiais gerados
- O Perfil de Artista deve ser criado apenas por quem **é o artista**, **integra a sua equipe** ou **o representa**. Ao criar um Perfil, você declara ter uma dessas relações ou informa expressamente que está apenas conhecendo a Plataforma; o registro dessa declaração é mantido com data, hora e endereço de IP.
- O diagnóstico combina **dados públicos medidos** em plataformas de streaming e métricas de audiência com **informações declaradas por você** — entre elas faturamento, cachê, número de shows, público pagante, premiações e repercussão de mídia. **A Maestra não apura nem verifica as informações declaradas**, e elas aparecem identificadas como tais nos materiais gerados.
- Os documentos exportados (inclusive em PDF) identificam **quem os gerou, com nome, e-mail e data e hora da geração**, e distinguem o dado medido do dado declarado. Ao exportar ou compartilhar o material, você concorda com essa identificação.
- É **vedado** utilizar a Plataforma para produzir, publicar ou fazer circular material que atribua a terceiro informação falsa, inclusive dados financeiros, de audiência ou de reconhecimento. A prática sujeita o infrator à remoção do Perfil e à suspensão ou encerramento da Conta, sem prejuízo das responsabilidades previstas nas cláusulas de Uso aceitável e de Indenização e das medidas cabíveis pelo titular prejudicado.
...
## 12. Inteligência artificial e análises automatizadas
...
## 13. Disponibilidade e isenção de garantias
...
- **Declarações e materiais gerados:** o registro da relação que você declara ter com cada Perfil de Artista (com data, hora e endereço de IP) e a identificação de autoria dos documentos que você exporta.

**Atenção aos materiais que você compartilha:** os documentos exportados da Plataforma, inclusive o diagnóstico em PDF, **contêm o seu nome, o seu e-mail e a data e hora da geração**. A identificação existe para dar responsabilidade e rastreabilidade ao material, evitando que diagnósticos sejam atribuídos a terceiros sem autoria conhecida. Ao enviar esses arquivos a outra pessoa, você compartilha também esses dados.
