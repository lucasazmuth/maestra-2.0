// ⚠️ CÓPIA de src/services/realEngine/index.ts (padrão de deploy do repo — Deno não importa fora
// do dir da função). Mantenha em SINCRONIA com o original (que tem os testes Jest).
// ─────────────────────────────────────────────────────────────────────────────
...
export const CUTS = {
  ...
  // ── A ──
  musicFollowers: { // [PROPOSTA] Spotify followers / Deezer fans; alto ≈ >250k
...
export function computeRealIndexV2(input: RealInputsV2): RealIndexV2 {
  ...
  const aComps: ComponentDebug[] = [
    comp('musicFollowers', 'Seguidores de plataformas de música', componentZ([
      ...
      apiZ(input.deezerFans, CUTS.musicFollowers, sp),
    ])),
