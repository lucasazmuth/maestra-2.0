...
/**
 * Clamps the size value between 1 and 512.
 * Falls back to 24 if undefined.
 */
function clampSize(size: number | undefined): number {
  if (size === undefined) return 24;
  return Math.max(1, Math.min(512, size));
}

/**
 * Returns a valid color string, falling back to 'currentColor' for empty/undefined values.
 */
function resolveColor(color: string | undefined): string {
  if (!color || color.trim() === '') return 'currentColor';
  return color;
}

/**
 * Dev-mode warning when an icon is used as sole interactive element
 * without aria-label and aria-hidden is not "true".
 */
function warnAccessibility(props: Record<string, any>, componentName: string): void {
  if (process.env.NODE_ENV !== 'production') {
    const ariaHidden = props['aria-hidden'];
    const ariaLabel = props['aria-label'];
    if (ariaHidden !== 'true' && ariaHidden !== true && !ariaLabel) {
      console.warn(
        `[Icon] <${componentName}> is used without an "aria-label" prop and "aria-hidden" is not "true". ` +
        `Provide an aria-label for accessibility or set aria-hidden="true" if the icon is decorative.`
      );
    }
  }
}
...
export const HomeIcon: FC<IconProps> = ({
  ...
  warnAccessibility(rest, 'HomeIcon');
  return (
    <svg
      data-encore-id='icon'
      role='img'
...
export const BrowseIcon: FC<IconProps> = ({
  size,
  color,
  className,
  ...rest
  ...
  const resolvedSize = clampSize(size);
  const resolvedColor = resolveColor(color);
  warnAccessibility(rest, 'BrowseIcon');
  return (
    <svg
      data-encore-id='icon'
      role='img'
...
export const SearchIcon: FC<IconProps> = ({
  ...
  const resolvedSize = clampSize(size);
  const resolvedColor = resolveColor(color);
  warnAccessibility(rest, 'SearchIcon');
  return (
    <svg
      data-encore-id='icon'
      role='img'
...
export const LibraryIcon: FC<IconProps> = ({
  size,
  color,
  className,
  ...rest
  ...
  const resolvedSize = clampSize(size);
  const resolvedColor = resolveColor(color);
  warnAccessibility(rest, 'LibraryIcon');
  return (
    <svg
      data-encore-id='icon'
      role='img'
...
export const LibraryCollapsedIcon: FC<IconProps> = ({
  size,
  color,
  className,
  ...rest
  ...
  const resolvedSize = clampSize(size);
  const resolvedColor = resolveColor(color);
  warnAccessibility(rest, 'LibraryCollapsedIcon');
  return (
    <svg
      data-encore-id='icon'
      role='img'
...
export const ShuffleIcon: FC<IconProps & { active?: boolean }> = ({
  active,
  size,
  color,
  className,
  ...
  const resolvedStyle = size || color
    ? { ...ControlStyle, ...(size ? { height: size, width: size } : {}), ...(style || {}) }
    : { ...ControlStyle, ...(style || {}) };
  const resolvedFill = active ? undefined : (color ? resolveColor(color) : undefined);
  return (
    <svg
      style={resolvedStyle}
      data-encore-id='icon'
      ...
      focusable='false'
      viewBox='0 0 16 16'
      className={className !== undefined ? className : `hoverable-icon replay ${active ? 'active' : ''}`}
...
export const SkipBack: FC<IconProps> = ({
  ...
  const resolvedStyle = size || color
    ? { ...ControlStyle, ...(size ? { height: size, width: size } : {}), ...(style || {}) }
    : { ...ControlStyle, ...(style || {}) };
  const resolvedFill = color ? resolveColor(color) : undefined;
  return (
    <svg
      style={resolvedStyle}
      data-encore-id='icon'
      ...
      focusable='false'
      viewBox='0 0 16 16'
      className={className !== undefined ? className : 'hoverable-icon'}
      {...(resolvedFill ? { fill: resolvedFill } : {})}
      {...rest}
    >
...
export const SkipNext: FC<IconProps> = ({
  size,
  color,
  className,
  style,
...
export const Replay: FC<IconProps & { active?: boolean }> = ({
  active,
  size,
  color,
  className,
...
export const ReplayOne: FC<IconProps & { active?: boolean }> = ({
  active,
  size,
  color,
  className,
...
export const Pause: FC<IconProps> = ({
  ...
  const baseStyle = { ...ControlStyle, fill: 'black' };
  const resolvedStyle = size || color
    ? { ...baseStyle, ...(size ? { height: size, width: size } : {}), ...(color ? { fill: resolveColor(color) } : {}), ...(style || {}) }
    : { ...baseStyle, ...(style || {}) };
  return (
    <svg
      role='img'
...
export const DetailsIcon: FC<IconProps & { active?: boolean }> = ({
  active,
  size,
  color,
  className,
  ...
  const resolvedStyle = size || color
    ? { ...SongExtraControlStyle, ...(size ? { height: size, width: size, maxWidth: undefined } : {}), ...(style || {}) }
    : { ...SongExtraControlStyle, ...(style || {}) };
  const resolvedFill = active ? undefined : (color ? resolveColor(color) : undefined);
  return (
    <svg
      style={resolvedStyle}
...
export const ListIcon: FC<IconProps & { active?: boolean }> = ({
  active,
  size,
  color,
  className,
...
export const DeviceIcon: FC<IconProps & { active?: boolean }> = ({
  active,
  size,
  color,
  className,
...
export const ExpandIcon: FC<IconProps> = ({
  ...
  return (
    <svg
      ...
      focusable='false'
      viewBox='0 0 16 16'
      className={className !== undefined ? className : 'hoverable-icon expand-icon'}
      style={resolvedStyle}
      {...(resolvedFill ? { fill: resolvedFill } : {})}
      {...rest}
    >
...
export const VolumeIcon: FC<IconProps> = ({
  size,
  color,
  className,
  style,
...
export const VolumeMuteIcon: FC<IconProps> = ({
  size,
  color,
  className,
  style,
...
export const VolumeOneIcon: FC<IconProps> = ({
  size,
  color,
  className,
  style,
...
export const VolumeTwoIcon: FC<IconProps> = ({
  size,
  color,
  className,
  style,
...
export const ExpandOutIcon: FC<IconProps> = ({
  size,
  color,
  className,
  style,
...
export const GridIconLegacy = (props: any) => (
  <svg
    data-encore-id='icon'
    role='img'
    aria-hidden='true'
...
export const GridIcon: FC<IconProps> = ({ size, color, className, ...rest }) => {
  const s = clampSize(size);
  const c = resolveColor(color);
  warnAccessibility(rest, 'GridIcon');
  return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" role="img" aria-hidden="true" focusable="false" className={className} {...rest}>
      <rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/>
    </svg>
  );
};

export const ActivityIcon: FC<IconProps> = ({ size, color, className, ...rest }) => {
  const s = clampSize(size);
  const c = resolveColor(color);
  warnAccessibility(rest, 'ActivityIcon');
  return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" role="img" aria-hidden="true" focusable="false" className={className} {...rest}>
...
export const TargetIcon: FC<IconProps> = ({ size, color, className, ...rest }) => {
  const s = clampSize(size);
  const c = resolveColor(color);
  warnAccessibility(rest, 'TargetIcon');
  return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" role="img" aria-hidden="true" focusable="false" className={className} {...rest}>
...
export const CheckSquareIcon: FC<IconProps> = ({ size, color, className, ...rest }) => {
  const s = clampSize(size);
  const c = resolveColor(color);
  warnAccessibility(rest, 'CheckSquareIcon');
  return (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" role="img" aria-hidden="true" focusable="false" className={className} {...rest}>
