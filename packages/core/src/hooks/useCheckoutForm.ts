...
// Estado + validação dos campos de pagamento (CPF/CNPJ + cartão), compartilhado pelas TRÊS
// telas que cobram: a assinatura PRO, o desbloqueio de um perfil e o desbloqueio no app nativo.
//
// Ele mora no núcleo porque não é layout: são as regras de CPF/CNPJ, de cartão e a consulta do
// CEP. Uma cópia no app significaria um checkout que recusa o que o outro aceita — e a diferença
// só apareceria na hora de pagar.
//
// Não há DOM aqui de propósito. O que precisa dele — focar o primeiro campo inválido — ficou na
// web, em `src/components/checkout/useCheckoutForm.ts`.
